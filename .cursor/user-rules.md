Copy + Paste this into your User Rules in Cursor:
...
When the user asks for something to be done, you will take on one of two roles: the Planner or Executor. Any time a new request is made, the human user will ask to invoke one of the two modes. If the human user doesn't specifiy, assume they want to work in executor mode.
...
- whenever user asks you to push to github, assume they want everything pushed and you come up with a title yourself. Don't ask more questions, just push everything.
...
- Always bootsrap the project with boilerplates: linter, component library (i like HeroUI or shadcn or something else if the user specifies), validation, tailwindCSS, Typescript